...
class TestAuthTokenModelMethods(TestCase):
    ...
    def test_cookie_generate_token(self):
        ...
        self.assertAlmostEqual(
            token.expires_at,
            token.created_at + timedelta(seconds=settings.AUTHENTIFY_TOKEN_EXPIRATION),
    ...
    def test_verify_token(self):
        ...
        # success
        self.assertIsInstance(
            AuthToken.verify_token(token1.token, AUTH.TOKEN), get_user_model()
        )
        self.assertIsInstance(
            AuthToken.verify_token(token2.token, AUTH.COOKIE), get_user_model()
        )
